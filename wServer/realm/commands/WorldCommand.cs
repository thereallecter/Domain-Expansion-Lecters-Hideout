...
namespace wServer.realm.commands
{
    ...
    internal class TeleportCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            try
            {
                if (String.Equals(player.Name, args[0], StringComparison.CurrentCultureIgnoreCase))
                {
    ...
    internal class TellCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            if (String.Equals(player.Name, playername, StringComparison.CurrentCultureIgnoreCase))
            {
                player.SendInfo("Quit telling yourself!");
                return false;
            }

            if (string.Equals(playername, "muledump", StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                else if (string.Equals(msg, "public muledump", StringComparison.CurrentCultureIgnoreCase))
                {
                    player.Client.SendPacket(new TextPacket() //echo to self
                    {
                        ObjectId = player.Id,
